...
import { SearchBoxContainer, StyledSearchBox, InputContainer, SearchIcon, Input } from './style';
...
const onStarClick = async () => {
  ...
  if (store.overlay.isBookmarked) {
    while (store.overlay.isBookmarked) {
      let tmp = store.bookmarks.list.find(x => x.url === store.tabs.selectedTab.url,)
      store.bookmarks.list.splice(store.bookmarks.list.indexOf(tmp), 1);
    }
  }
  else {
